...
Semantic colour map for population-genetics figures (PCA, admixture, FST heatmap, Manhattan, regional plots).

```python
POPGEN_PALETTE = {
    # 10-colour colourblind-safe qualitative set (populations / breeds)
    # Based on Paul Tol's "Bright" scheme; safe for deuteranopia and protanopia
    "qualitative": [
        "#4477AA",  # blue
        "#EE6677",  # red
        "#228833",  # green
        ...
        "#66CCEE",  # cyan
        "#AA3377",  # purple
        "#BBBBBB",  # grey (use last; reserve for "other" / unassigned)
        "#332288",  # indigo
        "#EE3377",  # magenta
        "#99EE66",  # lime
    ],

    # Diverging: FST, pi-ratio, Tajima's D, selection score
    # Anchor = neutral grey at 0; negative = blue, positive = red
    "diverging_low":  "#2166AC",   # strong negative
    "diverging_mid":  "#F7F7F7",   # neutral (anchor at 0 or median)
    "diverging_high": "#D6604D",   # strong positive

    # Sequential: p-value intensity, depth, score magnitude
    # Use matplotlib "magma" or "viridis" directly; these are fallback anchors
    "seq_low":  "#FFFFFF",
    "seq_high": "#4B0082",

    # Special roles
    "highlight":    "#E63946",   # candidate gene / sweep region (single callout only)
    "nonsig":       "#BDBDBD",   # non-significant SNPs (Manhattan background)
    "ancestral":    "#F4A261",   # ancestral allele / outgroup
}
```

**Usage rules:**
- Assign `qualitative[i]` consistently across all figures in a paper -- same breed/population always gets the same colour.
- For admixture bar plots, use the same `qualitative` colours as the corresponding PCA scatter; visual cross-referencing is a reviewer expectation.
- Never use `highlight` for more than one feature per panel.
- For diverging maps (FST heatmap), set `vmin`/`vmax` symmetrically around 0 or the median, not the data min/max.
